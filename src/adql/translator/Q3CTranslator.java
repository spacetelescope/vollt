...
/**
 * Translates all ADQL objects into the SQL adaptation of Postgres+Q3C.
 *
 * <p>
...
public class Q3CTranslator extends PostgreSQLTranslator {
	...
	/**
	 * Builds a Q3CTranslator which always translates in SQL all identifiers (schema, table and column) in a case sensitive manner ;
	 ...
	 */
	public Q3CTranslator(){
		super();
	}

	/**
	 * Builds a Q3CTranslator which always translates in SQL all identifiers (schema, table and column) in the specified case sensitivity ;
	 ...
	 */
	public Q3CTranslator(boolean allCaseSensitive){
		super(allCaseSensitive);
	}

	/**
	 * Builds a Q3CTranslator which will always translate in SQL identifiers with the defined case sensitivity.
	 *
	 ...
	 */
	public Q3CTranslator(boolean catalog, boolean schema, boolean table, boolean column){
	 ...
	 *
	 * @return	The Q3C serialization of the corresponding polygon.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * <p>Let parse a geometry serialized with the Q3C syntax.</p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * 	This parser supports all the known Q3C representations of an angle.
	...
	protected static class Q3CGeometryParser {
		/** Position of the next characters to read in the Q3C expression to parse. */
		private int pos;
		/** Full Q3C expression to parse. */
		private String expr;
		...
		private static class EOEException extends ParseException {
			...
			public EOEException(){
				super("Unexpected End Of Q3C Expression!");
			}
		}

		/**
		 * Build the geometry parser.
		 */
		public Q3CGeometryParser(){}

		/**
		 * Prepare the parser in order to read the given Q3C expression.
		 *
		 * @param newStcs	New Q3C expression to parse from now.
		 */
		 ...
		 * No more characters (except eventually some space characters) should remain in the Q3C expression to parse.
		 *
		...
		private void end() throws ParseException{
			...
			if (expr.length() > 0 && pos < expr.length())
				throw new ParseException("Unexpected end of Q3C region expression: \"" + expr.substring(pos) + "\" was unexpected!", new TextPosition(1, pos, 1, expr.length()));

			// Reset the buffer, token and the Q3C expression to parse:
			buffer = null;
			expr = null;
			token = null;
		}
		...
		/**
		 * Parse the given Q3C geometry as a point.
		 *
		 * @param q3cExpr	The Q3C expression to parse as a point.
		 *
		 * @return	A {@link Region} implementing a STC Position region.
		 *
		 * @throws ParseException	If the Q3C syntax of the given expression is wrong or does not correspond to a point.
		 */
		public Region parsePoint(final String q3cExpr) throws ParseException{
			// Init the parser:
			init(q3cExpr);
		...
		/**
		 * Internal spoint parsing function. It parses the q3c expression stored in this parser as a point.
		 *
		...
		/**
		 * Parse the given q3c geometry as a circle.
		 *
		 * @param q3cExpr	The Q3C expression to parse as a circle.
		 *
		 * @return	A {@link Region} implementing a STC Circle region.
		 *
		 * @throws ParseException	If the Q3C syntax of the given expression is wrong or does not correspond to a circle.
		 */
		public Region parseCircle(final String q3cExpr) throws ParseException{
			// Init the parser:
			init(q3cExpr);
		...
		/**
		 * Parse the given Q3C geometry as a box.
		 *
		 * @param q3cExpr	The Q3C expression to parse as a box.
		 *
		 * @return	A {@link Region} implementing a STC Box region.
		 *
		 * @throws ParseException	If the Q3C syntax of the given expression is wrong or does not correspond to a box.
		 */
		public Region parseBox(final String q3cExpr) throws ParseException{
			// Init the parser:
			init(q3cExpr);
		 ...
		 * @throws ParseException	If the Q3C syntax of the given expression is wrong or does not correspond to a point.
		 */
		public Region parsePolygon(final String q3cExpr) throws ParseException{
			// Init the parser:
			init(q3cExpr);
